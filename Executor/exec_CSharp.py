...
class CSharpExecutor:
    ...
    def compile_csharp(self, code, tmpdirname):
        filename = os.path.join(tmpdirname, 'Program.cs')
        executable = os.path.join(tmpdirname, 'Program.exe')

        with open(filename, 'w') as file:
            file.write(code)
        subprocess.run(['csc', '/out:' + executable, filename], check=True)
    ...
    def code_test(self, code, tests, answers):
        ...
        with tempfile.TemporaryDirectory(dir=self.tmp_dir) as tmpdirname:
            try:
                executable = self.compile_csharp(code, tmpdirname)
                wrong_cases = 0
                runtimes = 0
                memories = 0
                outputs = []
