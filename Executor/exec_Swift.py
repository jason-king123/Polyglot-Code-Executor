...
class SwiftExecutor:
    ...
    def compile_swift(self, code, tmpdirname):
        filename = os.path.join(tmpdirname, 'main.swift')
        executable = os.path.join(tmpdirname, 'main')

        with open(filename, 'w') as file:
            file.write(code)
        subprocess.run(['swiftc', filename, '-o', executable], check=True)
    ...
    def code_test(self, code, tests, answers):
        ...
        with tempfile.TemporaryDirectory(dir=self.tmp_dir) as tmpdirname:
            try:
                executable = self.compile_swift(code, tmpdirname)
                wrong_cases = 0
                runtimes = 0
                memories = 0
                outputs = []
