...
class PythonExecutor:
    ...
    def compile_python(self, code, tmpdirname):
        filename = os.path.join(tmpdirname, 'program.py')
    ...
    def measure_runtime_and_memory(self, script, test_data, timeout=5):
    ...
    def code_test(self, code, tests, answers):
        ...
        with tempfile.TemporaryDirectory(dir='./tmp') as tmpdirname:
            try:
                script = self.compile_python(code, tmpdirname)
                wrong_cases = 0
                runtimes = 0
                memories = 0
                outputs = []
                ...
                for test_data, answer in zip(tests, answers):
                    try:
                        runtime, runmemory, stdout = self.measure_runtime_and_memory(script, test_data)
