...
class PHPExecutor:
    ...
    def run_php(self, code, tmpdirname):
        filename = os.path.join(tmpdirname, 'script.php')
    ...
    def measure_runtime_and_memory(self, filename, test_data, timeout=5):
        ...
        process = subprocess.Popen(['php', filename], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    def code_test(self, code, tests, answers):
        ...
        with tempfile.TemporaryDirectory(dir=self.tmp_dir) as tmpdirname:
            try:
                filename = self.run_php(code, tmpdirname)
                wrong_cases = 0
                runtimes = 0
                memories = 0
                outputs = []
