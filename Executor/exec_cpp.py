...
class CppExecutor:
    ...
    def compile_cpp(self, code, tmpdirname):
        filename = os.path.join(tmpdirname, 'program.cpp')
        executable = os.path.join(tmpdirname, 'program')

        with open(filename, 'w') as file:
            file.write(code)
        subprocess.run(['g++', filename, '-o', executable], check=True)
    ...
    def code_test(self, code, tests, answers):
        ...
        with tempfile.TemporaryDirectory(dir='./tmp') as tmpdirname:
            try:
                executable = self.compile_cpp(code, tmpdirname)
                wrong_cases = 0
                runtimes = 0
                memories = 0
                outputs = []
