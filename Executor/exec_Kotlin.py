...
class KotlinExecutor:
    ...
    def compile_kotlin(self, code, tmpdirname):
        filename = os.path.join(tmpdirname, 'Main.kt')
        executable = os.path.join(tmpdirname, 'MainKt.class')

        with open(filename, 'w') as file:
            file.write(code)

        subprocess.run(['kotlinc', filename, '-include-runtime', '-d', tmpdirname], check=True)

        script_filename = os.path.join(tmpdirname, 'run.sh')
        with open(script_filename, 'w') as file:
            file.write(f'#!/bin/bash\n')
            file.write(f'java -cp "{tmpdirname}" MainKt\n')
    ...
    def code_test(self, code, tests, answers):
        ...
        with tempfile.TemporaryDirectory(dir=self.tmp_dir) as tmpdirname:
            try:
                executable = self.compile_kotlin(code, tmpdirname)
                wrong_cases = 0
                runtimes = 0
                memories = 0
                outputs = []
