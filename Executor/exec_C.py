...
class CExecutor:
    ...
    def compile_c(self, code, tmpdirname):
        filename = os.path.join(tmpdirname, 'program.c')
        executable = os.path.join(tmpdirname, 'program')

        with open(filename, 'w') as file:
            file.write(code)
        # Compile C code using gcc
        subprocess.run(['gcc', filename, '-o', executable], check=True)
    ...
    def code_test(self, code, tests, answers):
        ...
        with tempfile.TemporaryDirectory(dir=self.tmp_dir) as tmpdirname:
            try:
                executable = self.compile_c(code, tmpdirname)
                wrong_cases = 0
                runtimes = 0
                memories = 0
                outputs = []
