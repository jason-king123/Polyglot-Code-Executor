...
class RubyExecutor:
    ...
    def run_ruby(self, code, tmpdirname):
        filename = os.path.join(tmpdirname, 'script.rb')
    ...
    def measure_runtime_and_memory(self, filename, test_data, timeout=5):
        start_time = resource.getrusage(resource.RUSAGE_CHILDREN).ru_utime

        process = subprocess.Popen(['ruby', filename], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    def code_test(self, code, tests, answers):
        ...
        with tempfile.TemporaryDirectory(dir=self.tmp_dir) as tmpdirname:
            try:
                filename = self.run_ruby(code, tmpdirname)
                wrong_cases = 0
                runtimes = 0
                memories = 0
                outputs = []
                ...
                for test_data, answer in zip(tests, answers):
                    try:
                        runtime, runmemory, stdout = self.measure_runtime_and_memory(filename, test_data)
